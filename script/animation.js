...
THREE.Camera.prototype.locateTween = function(lookAtVector, direction, distance, theta, duration) {
	...
	var update = function() {
		self.locate(new THREE.Vector3(this.viewX, this.viewY, this.viewZ), new THREE.Vector2(this.directionX,
				this.directionY), this.d, this.t);
	}

	return new TWEEN.Tween(from).to(to, duration * $.SPEED).onUpdate(update).easing(TWEEN.Easing.Cubic.InOut);
}
...
THREE.Object3D.prototype.growTween = function(duration) {
	...
	var update = function() {
		...
		var scale = TWEEN.Easing.Elastic.Out(this.fact);
